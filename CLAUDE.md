...
- **Tests**: inline `#[cfg(test)] mod tests` blocks (this is a *binary* crate — a top-level `tests/` dir can't reach internal/private items like `escape` and `parse_ack`). 299 offline tests; none touch the network or need an MPD server (the `store` ones do open a real redb, over `InMemoryBackend`):
...
- **A section lists an entity only when its *own name* matches.** A search for `love` must not promote all 300 artists who recorded a song with "love" in the title into an Artists section; the Songs section is the one that keeps every match. Matching is `folded_contains`, which folds by the same rules as `album_grouping_key`, so a typed hyphen finds an en dash.
...
## Current Version
`0.5.0` — see `Cargo.toml`. There are `release` and `ship` skills that automate the release/merge flow — prefer them over doing the steps by hand.
