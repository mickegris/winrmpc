...
- **Known gap**: `album_songs`/`album_bios` are still keyed by base name alone (not artist-scoped) — two different artists' same-named album opened in sequence within one session can collide in those caches. Pre-existing limitation, not a regression; fixing it needs a broader key-shape change deferred to a follow-up.
