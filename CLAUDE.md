...
- **Tests**: inline `#[cfg(test)] mod tests` blocks (this is a *binary* crate — a top-level `tests/` dir can't reach internal/private items like `escape` and `parse_ack`). 295 offline tests; none touch the network or need an MPD server (the `store` ones do open a real redb, over `InMemoryBackend`):
