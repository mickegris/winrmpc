...
`ui-0.4.3.md` is the newest umbrella (targeting 0.4.3, **plans only so far**), linking four plans: `icon-buttons-and-transport-controls.md`, `clickable-artist-album-links.md`, `keyboard-shortcuts.md`, `album-highlighting-and-scroll-to-playing.md`. The last two were proposed rather than requested; the fourth finishes steps D/E deferred out of `current-song-highlighting.md`. Its "Considered and not planned" section records the ideas that were weighed and left out, so they don't get rediscovered from scratch.
