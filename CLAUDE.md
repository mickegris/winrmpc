...
- **Tests**: inline `#[cfg(test)] mod tests` blocks (this is a *binary* crate — a top-level `tests/` dir can't reach internal/private items like `escape` and `parse_ack`). 276 offline tests; none touch the network or need an MPD server (the `store` ones do open a real redb, over `InMemoryBackend`):
...
Artists, Genres and Playlists carry an **A–Z / Z–A** toggle. The album lists (Albums, Artist detail, Genre detail) carry a **Name / Year / Added** picker beside it. `AppConfig::sort_key` + `sort_desc` drive all of them — one preference, not one per list, the same call `album_grid_view` made.
...
- **A missing year or add-time sorts last in *both* directions**, and the unknown check sits *outside* the reversal. An album with no `Date` tag heading a "Newest first" list would read as data rather than as a gap — the same rule already governs `last_modified` in Recently Added. Consequence: for Year and Added, reversing twice is **not** a pure mirror, and the tests assert the weaker property (the knowns mirror, the unknowns stay at the end).
- **The year tiebreak is not reversed with the primary key.** A block of 1990 albums reading Z–A inside an "Oldest first" list looks like a bug.
- **Sorting happens in `update`, never in `view()`.** Views take `&'a [T]` and would have to allocate a reordered copy every frame. `App::sort_library_lists()` is called from both entry points — every `*Loaded` handler and the two sort messages — so they can't drift onto different comparators. It also **stamps `AlbumGroup::year` from `album_years` before sorting**, so a list loaded before the year query landed picks the years up on the next sort. Toggling **re-sorts in memory and does not refetch**.

### Where the years and add-times come from
The two are wildly different in cost, and the split between them is the point.
- **Years: `list Date group AlbumArtist group Album`, once, in `fetch_all()`.** `list` returns one line per *distinct* value, so a whole library is a few thousand lines — cheap enough to fetch unconditionally on connect. `parse_grouped_values2` reads the two nesting levels and **clears the inner group when the outer changes**: MPD emits a group header only when that group's value changes, so a remembered `Album` belongs to the previous `AlbumArtist` and would leak a year sideways onto another artist's album. `album_year_index` keeps the **earliest** year per album — a reissue's tracks often carry the reissue date while one or two keep the original, and the original is what a release year means on a row.
- **Add-times: a paged walk of every song, on demand only.** There is no `list` shortcut — `Added` is not a tag, so the only way to learn when an album was added is to look at its songs. `MpdClient::added_page` reuses the Recently Added rung ladder (so it degrades to `Last-Modified` on pre-0.24) and returns **raw pairs**: `fold_album_added` extracts three fields per song without ever building a `Song`, which is most of the cost and all of it waste here. Paged at `ADDED_PAGE` (10 000) so peak memory is one page rather than the whole library — the same hazard `RECENTLY_ADDED_LIMIT` exists for — and capped at `ADDED_MAX_PAGES` so a pathological library degrades to partial data instead of looping. It runs only when the Added sort is actually selected, and a partial index is kept on failure: every album it missed sorts into the unknown bucket rather than a wrong position.
- **Both indexes key on the raw `AlbumArtist` tag** (empty when absent), not `display_album_artist()`, because that is what `list Album group AlbumArtist` keys the album list itself on. A fallback here would build keys the album rows never look up. Both are disc-collapsed through `album_base_and_disc`, and both are cleared by `SwitchServer` — they describe one server's library.
- **The year is shown on every album row and tile**, because ordering a list by a column that isn't on screen leaves no way to tell the sort worked.
- **The nesting order is a live test, not an assumption**: `live_album_years_attach_to_the_right_albums` checks the year keys against the real album list, since a swapped nesting would silently attach every year to the wrong album.
...
## Current Version
`0.4.3` — see `Cargo.toml`. There are `release` and `ship` skills that automate the release/merge flow — prefer them over doing the steps by hand.
