...
- **Tests**: inline `#[cfg(test)] mod tests` blocks (this is a *binary* crate — a top-level `tests/` dir can't reach internal/private items like `escape` and `parse_ack`). 322 offline tests; none touch the network or need an MPD server (the `store` ones do open a real redb, over `InMemoryBackend`):
  - `mpd/client.rs` — `escape` injection safety (quotes, backslashes, ordering), `password_command` escaping, and that `MpdError::Auth` is not connection-fatal
  ...
  - `mpd/types.rs` — `album_grouping_key` (an en dash and a hyphen are one album; folding doesn't merge genuinely different ones), `search_sections` (only entities whose own name matches; both artist tags searched; placeholders never offered), `Song` display fallbacks, `format_duration`, `art_key` (0x1f separator, hyphen-collision guard), `display_format` (codec from extension), `push_recent` (dedup/move-to-front/cap-at-8)
...
packaging/                   Desktop-integration assets: linux/ (.desktop + hicolor icons + install.sh), macos/ (Info.plist template + bundle.sh), fonts/ (icon-font generator)
src/
...
- `recent_albums: Vec<RecentAlbum>` — most-recent-first, capped at 8, `#[serde(default)]`; updated on `CurrentSongUpdated` when the album changes. **Stored and de-duplicated on the disc-stripped, punctuation-folded base** (`RecentAlbum::grouping_key`, 0.5.0): crossing from disc 1 to disc 2 is not "a new album", and it used to take a second of only eight slots — one 3-disc set ate nearly half the strip and evicted four genuinely different albums. Storing the base rather than the raw tag also fixes the tile caption (`"X [Disc 2]"`) and navigation, since `AlbumSelected` looks for a group whose *base* matches and a suffixed name matches none, rendering that one disc instead of the set. `grouping_key` strips inside itself rather than trusting the caller, so a `recent_albums` list loaded from an older config file de-dups correctly too.
...
- `sort_desc: bool` + `sort_key: SortKey` (Name/Year/Added) — list ordering, both `#[serde(default)]`; see "List sorting"
...
| `sort_desc` / `sort_key` | the sort controls in each list header |
...
- **`theme.dark_mode` is live as of 0.4.3** — see "Light and dark" below. `accent_color` was **removed** rather than implemented.
...
## Outputs and partition moves (`src/mpd/commands.rs`, `Message::MoveOutput`)
**Moving an output is a five-step sequence, and each step is there for a reason** — ported from mikMPD, which had already been through this against a real server:
1. **Disable the output first.** `moveoutput` on an open, actively-rendering output **can deadlock the MPD server**. This is the step that makes a move safe rather than merely tidy, and it is the one that isn't obvious from the protocol docs.
2. **Switch to the target partition** — `moveoutput` moves into the *current* partition, not to a named one.
3. `moveoutput`.
4. **Re-enable by name, not by id.** MPD may assign a **new output id** after the move, so the old one is meaningless. Finding by name works only because `parse_outputs` filters the dummy (below), which shares the name.
5. **Always switch back** to the partition the user was in, even when the move failed — stranding them elsewhere is worse than the failed move.

There are 100ms settles between the partition-switching commands, and `App::moving_output` guards against a second move starting while the client is parked in another partition. Failures now surface as a toast instead of `.ok()`.

**`parse_outputs` drops entries whose `plugin` is `dummy`.** This is **not** a rare post-move artifact — it is the steady state of any multi-partition setup. On the 10.0.1.3 server the `default` partition reports **ten outputs of which nine are dummies**, one for every output that actually lives elsewhere; `live_outputs_never_include_dummy_placeholders` checks every partition against a real server. Moving an output to another partition leaves a same-named placeholder behind in the partition it came from, reported with `plugin: dummy`. It can't play anything, but in a list it is indistinguishable from the real output — so moving one back and forth appeared to *clone* it. Filtering in the parser means every consumer is clean rather than each view remembering to check, and the filter keys on the **plugin**, not the name: the ghost and the real output share a name, so a name-based filter would hide both.
...
- **`MpdClient::new(addr, password)` — the password is taken at construction and sent by `connect()`, and that placement is the whole fix.** MPD authenticates a *connection*, not a client, so every path that opens a socket has to re-send it: the initial connect, `ConnectionTick`'s reconnect, and the recovery that drops a desynced connection. From 0.4.0 to 0.4.5 `MpdClient::password()` existed and **nothing ever called it** — `config.servers[].password` was loaded, editable in Settings and saved to TOML, and read by no one, so a password-protected server was met with an unauthenticated connection. `AppConfig::server_password(name)` walks *exactly* the same fallback chain as `server_addr` (named server → first server → legacy fields); the two must agree, or a fallback address gets paired with no credentials. A blank password counts as absent — `password ""` is a wrong credential to MPD, not an empty one.
- **A rejected password fails the whole `connect`** (`MpdError::Auth`), and the connection is never published. It has to: MPD answers every later command with `ACK [4@0] … you don't have permission`, an ACK is deliberately *not* connection-fatal, and `ConnectionTick` short-circuits on `is_connected()` — so an unauthenticated-but-`Some` connection would make the app log "Connected to MPD" and then fail silently forever. That was the shipped symptom. `mock_mpd_requiring_password` in `live_tests.rs` covers all of it offline (the real test server wants no password, and one that does can't be asked to stop); `WINRMPC_TEST_MPD_PASSWORD` runs the live suite against a protected server.
...
- **`MpdClient::list_albums_by_artist()`** sends `list Album group AlbumArtist` (MPD 0.21+; falls back to a flat, artist-less list on ACK from older servers). `group_albums_by_artist(pairs)` groups the resulting `(artist, album)` pairs, keyed by **`album_grouping_key(artist, base)`** — `base` comes from `album_base_and_disc`.
- **`album_grouping_key` is the one album-identity function, and every consumer must go through it** or "a row won't find its own disc data" (mikMPD's phrasing; it learned this the hard way). Today that means `group_albums_by_artist`'s map key and the variant lookup in `AlbumSelected` (`app.rs`), which re-derives the artist's groups and has to match the base it was handed — a plain `g.base == album_name` there re-splits on exactly what the key folds, and the row silently renders one disc instead of the set. It folds **case** (the first-seen spelling is still what gets displayed) — inconsistent capitalisation across one album's discs is common, and without it `"Decade Of Aggression - Disc 2"` and `"Decade of Aggression - Disc 1 of 2"` are two rows — **and punctuation**: en/em dash/minus/non-breaking hyphens → `-`, smart quotes → straight, `…` → `...`, whitespace runs → one space. The punctuation half landed in 0.5.0 (deferred as `review-fixes-correctness.md` §6 since 0.4.1) and the user's own library is the proof: The Beatles' `1967-1970` (ASCII hyphen, one disc) and `1967–1970` (en dash, the other) were two rows, each captioned as a single disc and each holding half the set. Deliberately **not** `normalize_for_lookup`, which does all of this and then moves a sort-order article to the front — right for asking a search engine a question, wrong for identity, and `types.rs` has no business depending on the lookup layer.
- **`art_key_for` is *not* folded, and that is a known gap rather than an oversight.** It disc-strips but preserves case and punctuation, so a per-track key built from a raw tag (`Song::art_key()`, which Now Playing uses) can differ from the group-level key the grid stores under — the cover then appears in one place and not the other. It already split on case long before the punctuation folding existed; `live_album_grouping_collapses_real_multidisc_albums` now *prints* every such split rather than asserting (6 of 45 multi-disc groups on the real library, 5 of them case-only). Folding it would re-key every cached cover in every existing install, which is a migration, not a fix — see `docs/status.md`.
...
- **`display_artist()` falls back to the album artist**, and `display_album_artist()` to the track artist — mirror images, so the two agree on any file carrying only one of the tags. **A present-but-blank tag counts as absent** (`tag_or`), since a chain that stops at `""` renders an empty cell, which reads as a rendering fault rather than missing data. `tag_or` returns the *original* value rather than a trimmed one: trimming would change `display_album_artist`, which feeds `art_key`, and silently orphan cached art for any file with a padded tag. Files carrying `AlbumArtist` and no `Artist` are common (a rip where only the album-level tag was written); those read as "Unknown Artist" in the queue and every track list while the album page — which uses `display_album_artist()` — showed the name fine. The two methods now agree. It also matters beyond display: `display_artist()` is what's sent to LRCLIB and MusicBrainz, so the fallback turns a guaranteed-miss lookup into one that can match.
...
- **`AlbumGroup::disc_count()` / `disc_caption()` are what the list rows show, and `variants.len()` is not the answer.** A variant is any distinct raw album tag that folded into the group, and the key folds case and punctuation — so a library holding both `"Love at First Sting"` and `"Love At First Sting"` produced a two-variant group and a row captioned **"2 discs"** for a single-disc album (the reported 0.5.1 bug; 5 rows on the real library, and `live_album_grouping_collapses_real_multidisc_albums` prints each one it fixes). Only variants carrying an actual disc marker count, as the max of the highest disc number and the count of *distinct* ones — neither alone works, since `["X", "X [Disc 2]"]` is two discs with one marker while `["X [Disc 1]", "X [disc 1]"]` is one disc written twice. It **under-reports rather than over-reports** on purpose: a set distinguished only by a `disc` tag reports 1, because nothing in the names says otherwise. A missing caption is a gap; a wrong one is a claim. All five caption sites (Albums, Artist, Genre detail, Search, the grid tile) go through it.
...
- **Recently Added** (`View::RecentlyAdded`, sidebar beneath Genres): `MpdClient::find_recently_added(since, limit)` → `(Vec<Song>, RecentlyAddedRung)`. Always bounded (`RECENTLY_ADDED_LIMIT` = 5000 songs over `RECENTLY_ADDED_DAYS` = 90) — an unbounded scan can outrun the socket read on a large library. Songs are collapsed to unique album names and rendered through the same `views::albums_list::view` as the plain Albums list (`title` + optional `subtitle` params).
  - **The query must sort server-side, and that is the whole point of `RecentlyAddedRung`.** Until 0.4.4 it was `find "(modified-since '…')" window 0:2000`, and MPD returns `find` matches in **database order** — documented as undefined, in practice directory traversal — with `window` slicing *that*. The client then sorted the survivors by `last_modified`. So past 2000 matches, *which* albums appeared was decided by path order and a newly added album whose path sorted late never reached the client; sorting after truncation cannot recover a row the server never sent. It failed silently and only above the cap, which is why it read as "Recently Added stopped working".
  - **`sort` runs before `window` in MPD**, so a descending sort makes the limit drop the *oldest* matches. `RecentlyAddedRung` is the three-rung ladder: `find "(added-since …)" sort -Added` (0.24+), `find "(modified-since …)" sort -Last-Modified` (0.22+), then the old unsorted form. `query()` lives on the enum in `types.rs` so it's a pure, unit-testable function.
  - **`Added` vs `Last-Modified` is not just syntax.** `modified-since` is file mtime, which lies both ways: `cp -p`/`rsync -a`/a restored backup carry the original date forward so files added today can be years old and never appear, while editing a tag on an old file resurfaces it as "recently added". 0.24's `Added` is a real database add-time.
  - **Only an ACK falls through to the next rung.** A connection error propagates — walking the ladder on a dead socket would cache a rung the server never rejected and silently downgrade the view for the rest of the session. The rung is cached in an `Arc<AtomicU8>` shared across client clones (`0` = unprobed) and **cleared by `connect()`**, so an upgraded server isn't stuck on the old rung.
  - **The pre-0.24/pre-0.22 paths are tested offline against a mock MPD** (`mock_mpd_rejecting` in `live_tests.rs`) — they can only be exercised against a server *lacking* the newer syntax, and the real test server runs 0.24.
  - A result that hits the limit exactly logs at INFO with the oldest date shown, and an unsorted rung logs a WARN; a truncated list was previously indistinguishable from a complete one. The header carries "· last 90 days" for the same reason. Failures reach `toast_error` — `.unwrap_or_default()` used to render a failed query as an empty section.
...
## List sorting (`src/mpd/types.rs`, `App::sort_library_lists`)
Artists, Genres and Playlists carry an **A–Z / Z–A** toggle. The album lists (Albums, Artist detail, Genre detail) carry a **Name / Added** picker beside it. `AppConfig::sort_key` + `sort_desc` drive all of them — one preference, not one per list, the same call `album_grid_view` made.
- **`name_cmp` is not `str::cmp`, and that was a real bug.** A plain `Vec<String>::sort()` is byte order, so every lowercase initial filed after every uppercase one: `ZZ Top` (`Z` = 0x5A) sorted before `a-ha` (`a` = 0x61). `a-ha`, `dEUS`, `k.d. lang` and `will.i.am` were all exiled past Z in the Artists list. `name_cmp` case-folds, **then breaks ties on bytes** — the tiebreak keeps the order *total*, without which names differing only in case have no defined order and reversing twice shuffles the list instead of restoring it.
- **Albums sort by the album name, not artist-then-name.** An A–Z album list is read down the column of titles; ordering by artist first makes it look unsorted to anyone doing that. The artist is the tiebreak, so two artists' same-titled albums stay deterministically ordered.
- **`album_cmp` takes the add-times as parameters** rather than reading them off `AlbumGroup`, because they arrive from a completely different and far more expensive query than the album list. That keeps it a pure function.
- **A missing add-time sorts last in *both* directions**, and the unknown check sits *outside* the reversal. An album the walk never reached heading a "Newest first" list would read as data rather than as a gap — the same rule already governs `last_modified` in Recently Added. Consequence: for Added, reversing twice is **not** a pure mirror, and the test asserts the weaker property (the knowns mirror, the unknowns stay at the end).
- **The tiebreak is not reversed with the primary key.** A block of same-dated albums reading Z–A inside an "Oldest first" list looks like a bug.
- **Sorting happens in `update`, never in `view()`.** Views take `&'a [T]` and would have to allocate a reordered copy every frame. `App::sort_library_lists()` is called from both entry points — every `*Loaded` handler and the two sort messages — so they can't drift onto different comparators. Toggling **re-sorts in memory and does not refetch**.

### Where the add-times come from
**A paged walk of every song, on demand only.** There is no `list` shortcut — `Added` is not a tag, so the only way to learn when an album was added is to look at its songs. `MpdClient::added_page` reuses the Recently Added rung ladder (so it degrades to `Last-Modified` on pre-0.24) and returns **raw pairs**: `fold_album_added` extracts three fields per song without ever building a `Song`, which would be most of the cost and all of it waste here. Paged at `ADDED_PAGE` (10 000) so peak memory is one page rather than the whole library — the same hazard `RECENTLY_ADDED_LIMIT` exists for — and capped at `ADDED_MAX_PAGES` so a pathological library degrades to partial data instead of looping. It runs only when the Added sort is actually selected, and a partial index is kept on failure: every album it missed sorts into the unknown bucket rather than a wrong position.
- **It keys on `AlbumArtist` falling back to `Artist`, because MPD's own grouping does.** `list Album group AlbumArtist` reports `Dio` for an album whose song records carry `Artist: Dio` and **no `AlbumArtist` at all** — the server substitutes the fallback tag, and 0.4.4 shipped a fold that didn't, so it built `"\x1fHoly Diver"` against a row keyed `"Dio\x1fHoly Diver"`. On the real library that was **453 of 801 album rows** silently sorting into the unknown bucket. Disc-collapsed through `album_base_and_disc`, and cleared by `SwitchServer` since it describes one server's library.
- **The client must sort by the field its *rung* ordered on**, not always `last_modified`. 0.4.4 asked for `sort -Added` and then re-sorted the reply by mtime, discarding the ordering one line after it arrived. On the test server every file reports the same `Last-Modified` (a mass rewrite) while `Added` spans eight months, so mtime is close to worthless there — which is the whole reason the 0.24 rung exists. `Song::added` carries the field; `#[serde(default)]`, since older servers never send it.
- **Both defects were found by the live tests and neither is visible offline** — a mock server can't disagree with itself about `AlbumArtist`, and a fixture can't have a uniform mtime. `live_diagnose_album_added_coverage` prints the mismatched keys and is the fastest way back to this if it recurs.

### Which lists, and which not
- **The recency lists are deliberately excluded.** `recently_added_albums` and `recently_played` are ordered by time, which is the only thing they are for. `albums_list::view` is shared with Recently Added, so its `sort` param is `Option<(SortKey, bool)>` rather than a pair — `None` means the control is **absent**, not present and doing nothing.
- **Also excluded, each for its own reason**: the Queue (the order *is* the queue — sorting the display would decouple rows from the positions the move/remove actions use), Album detail (`(effective_disc, track)` is the album's own order), Search (relevance-ranked by `fuzzy-matcher`; alphabetising throws the ranking away), Browser (a directory listing's order is the server's, the same reason its rows carry no track number).
- **The direction control is text, and its wording follows the key** (`SortKey::direction_label`) — "A–Z" is meaningless for a date and "Oldest" is meaningless for a title. The obvious glyph reuse is barred anyway: `icon::MOVE_UP`/`MOVE_DOWN` already *are* `arrow_upward`/`arrow_downward`, and `icon::tests` asserts no two constants share a codepoint.
- **Tried and removed: sorting by release year.** It worked off `list Date group AlbumArtist group Album` — cheap (one line per distinct value, not per song) but resting on an assumption about how MPD nests two `group` levels that was never confirmed against a real server. A swapped nesting attaches every year to the wrong album, silently and plausibly. Removed rather than shipped unverified; the display of the year went with it, since it drew on the same query. Re-adding means confirming the nesting first.
- **Not done: ignoring a leading article** so "The Beatles" files under B. That's an opinionated, locale-flavoured transform, and `normalize_for_lookup` already moves the article the *other* way for external lookups, so reusing it here would be actively confusing.

## Transient feedback — toasts (`src/ui/app.rs`)
`last_error` was written from seven places and rendered from **one**, inside `settings_view()` — so an error was only visible if the user happened to be standing on the Settings screen when it happened. Failing to save a playlist from the Playlists view produced no visible response at all.
- **`App::toast_error(text)` / `toast_info(text)` are how anything user-visible is reported.** Both still set `last_error` (it backs the Settings status line), but the toast is the half that's visible from wherever the user actually is. Don't write `last_error` directly for anything worth noticing.
- **Errors and info are distinguished.** The old field was named `last_error` yet also carried `"Database update started"`; the two now differ in styling *and* in how long they stay up (8s vs 4s — an error is the one worth reading, and a test pins that ordering).
- **A queue, not a slot** (`VecDeque`, cap `MAX_TOASTS` = 3). Two failures in quick succession — a bulk enqueue hitting several bad URIs — must not have the second silently replace the first before it's read. When full the *oldest* is dropped, since the newest failure is the one being looked for.
- **Expiry rides the existing 500ms `Tick`.** A dedicated timer for a 4-second toast would be waste.
- **Rendered in an `iced::widget::stack` over the content**, bottom-right, clear of the player bar. Floating rather than displacing means a message can't reflow the view underneath while it's being read; the overlay column is `Shrink` so it doesn't cover — or swallow clicks meant for — the rest of the window. Clicking a toast dismisses it.
...
## Clickable artist / album names (`src/ui/widgets/link.rs`)
**Every artist and album name the app renders is a link to that artist or album.** `link::artist_link(name, size)` and `link::album_link(album, artist, size)` are the only ways to draw one, so no call site has to remember which `Message` to build.
- **`album_message(album, artist)` is the shared message builder**, used by `album_link` *and* by every container that navigates to an album (the cover tile, the list rows). A row that navigates to an album must not assemble its own `AlbumSelected`, or it will eventually forget the placeholder rule below.
- **The missing-tag placeholders are never links.** `display_artist()`/`display_album()` fall back to `"Unknown Artist"`/`"Unknown Album"`; navigating to those renders an empty page, so `is_real_name()` gates them down to inert muted text. `album_message` also strips a placeholder *artist* to `None` rather than passing it through.
- **A name link is never nested inside a bigger button.** `album_grid::tile` and the album list rows used to wrap cover + title + artist in one album-wide button, which left the artist as a small region of a large clickable area doing something different, invisibly. Cover + album title are now one button and the artist is a **sibling** link. Same fix in Now Playing's Up Next row and Recently Played's track rows. Cover+title stays one target so the common action keeps a big, forgiving hit area.
- Links are muted at rest, accent on hover, and **not underlined** — at size 11-12 in a dense list, underlining every artist turns it into a thicket.

**Genre detail is artist-scoped like everything else** (`MpdClient::list_albums_by_artist_filtered`). It used to send `AlbumSelected(name, None)`, and that artist-less path skips multi-disc expansion — so opening a multi-disc album from a genre showed **only the tracks whose literal `Album` tag matched the collapsed base name**. `genre_albums` is now `Vec<AlbumGroup>`, which also disc-collapses the genre listing itself. The pre-0.21 `group` fallback degrades to a flat artist-less list, matching `list_albums_by_artist`.
...
- **Album-level highlighting** (`song_row::is_current_album`) marks the album *containing* the playing track, in Albums, Recently Added, Recently Played, Artist detail and Genre detail. It is **not** `album == album`: the comparison is against the *disc-collapsed* base, because a playing `Disc 2` track must light up the single collapsed row that represents the set — the row on screen has no "Disc 2" in its name. Artist-scoped for the same reason album keys are, case-folded like the grouping, and **false for the missing-tag placeholders**, which is what stops a radio stream (whose `display_album()` is junk) marking every untagged album at once. Grid tiles get an accent border on the cover *plus* an accent title; the title alone is too subtle at grid density.
- **`Message::JumpToCurrent`** scrolls the Queue to the playing track, using the same `index / (len - 1)` ratio as the lyrics autoscroll — more accurate here, because queue rows are a uniform height and lyric lines aren't. **A button, never automatic**: unconditional autoscroll is exactly what made the lyrics pane unreadable before 0.4.2 gave it a Sync/Scroll switch. Shown only when there is a playing track in that queue. Currently Queue-only; the other lists are short enough that it hasn't earned its place there.
- **Superseded** (was deferred as plan step D): album-level highlighting — marking the album *containing* the playing track in the grids and album lists. It's a different comparison (`album_scoped_key` against the disc-collapsed base, so a playing Disc 2 track marks the single collapsed row) with its own edge cases, and landing it separately keeps an album-key bug from holding up the track-list work. Also out of scope (step E): auto-scrolling a list to the playing track, and the Radio view.

## Keyboard shortcuts (`src/ui/shortcut.rs`)
The app handled **no** key input at all before 0.4.3.
- **`on_key_press` takes a `fn` pointer, not a closure** (`iced_futures-0.13.2/src/keyboard.rs:13`), so the subscription can't capture app state. It emits a raw `Message::KeyPressed(Key, Modifiers)` and `update` decides — which is the better shape anyway, because `shortcut::resolve` is then a pure function over an explicit `Context` and the thing most likely to break is directly testable.
- **The gate is per-view, and that is forced.** iced 0.13's `text_input` has no focus/blur callbacks, and `operation::focusable::find_focused()` resolves through a `Task` — asynchronous, so it cannot gate a keypress being handled *now*. There is no synchronous way to ask "is a text box focused?". So bare keys are suppressed on the seven views that contain a `text_input` (Search, Settings, Radio, CD, Partitions, Playlists, AddToPlaylist) and modifier shortcuts work everywhere. **`bare_keys_never_fire_on_a_view_with_a_text_input` is the acceptance test** — a shortcut that eats a keystroke meant as text is worse than no shortcut.
- **Ctrl+↑/↓ for volume, not bare ↑/↓**: in a scrollable list the bare keys mean scroll, and iced routes them to the focused scrollable.
- **Esc is ungated** — it isn't a text character, and going Back from a view with a search box is reasonable.
- `Ctrl+F` / `/` focus the search box through a **stable** `text_input::Id` (`views::search::input_id()`), not `Id::unique()`, since another module has to name it; it also `select_all`s so the next keystroke replaces an existing query.
- **Settings lists them.** A shortcut nobody knows about is dead code.

## Window geometry (`AppConfig::window`, `main.rs`, `App`)
Size and maximised state are remembered between launches.
- **Position is deliberately *not* restored.** iced 0.13 exposes no monitor enumeration, so a saved position can't be validated against the displays that exist at restore time — and the failure is unrecoverable from inside the app: the window reopens on a monitor that's gone, is invisible, and the only fix is hand-editing `config.toml`. Size is the actual irritation; position is the part that can strand you.
- **`window_settings()` loads the config a second time**, because it runs in `main` before `App::new` exists. Reading a small TOML twice at startup beats restructuring startup around it.
- **A saved size below `min_size` is clamped, not honoured** (`WindowConfig::restored_size`, which also rejects NaN/zero from a hand-edited file). iced would let a smaller value through, and below roughly 1000×700 Now Playing stops fitting — iced widgets don't clip their parent, so too-small overlaps rather than degrading.
- **`window::Settings` has no `maximized` field in 0.13**, so it's applied after the window opens via `window::get_latest()` → `window::maximize`.
- **Close is intercepted** (`exit_on_close_request: false`) because the maximised state is only reachable through an async `Task`, and a normal close leaves nothing to query. ⚠️ **Every path through `WindowCloseRequested`/`WindowClosing` must end in `window::close`** — an unclosable window is a far worse bug than a forgotten size.
- **The size isn't overwritten while maximised**: platforms disagree about whether a maximised window reports its restored or its screen size, and saving the latter would make the next un-maximised launch screen-sized.
- Resizes update memory only; the write happens once, on close. A drag emits a resize per frame and rewriting the TOML each time would be pathological.

## Light and dark (`src/ui/theme/colors.rs`)
`theme.dark_mode` was persisted and read by nothing from 0.4.0 to 0.4.2. It is now real, applied immediately from Settings → Appearance, and remembered.
- **The colours are functions, not constants, and that is forced.** ~320 of the uses are direct `.color(...)` calls in view code, and iced only hands `&Theme` to a *style closure* — so those sites have no theme in scope. Threading a `&Palette` through every `views::*::view()` signature would touch every view and every call site in `app.rs` to forward a value. Instead the mode is one process-global `AtomicBool` and `AppColors::text_primary()` indexes a two-element table. **`set_dark_mode` is called from exactly two places** — startup and the Settings toggle.
- **`Palette` is a struct with both palettes as `const`s**, so adding a colour to one without the other doesn't compile.
- **`App::theme()` builds an `iced::Theme::custom` from the same `Palette`.** Swapping only `AppColors` would give a light app with **dark dropdowns, text inputs and sliders** — those come from iced's own theme. Both halves derive from one source, and the two themes are cached in `OnceLock`s because `theme()` runs every redraw and `Theme::custom` allocates.
- **The light palette is not an inversion.** Dark's `accent` (`#4fc3f7`) is near-illegible on white *and* is the playing-track text colour, so light gets a much darker blue; `row_playing` is a blue **tint** because on a light background the three row colours sit within a few percent of each other and a neutral step would vanish; `warning`/`success` are darkened well past their dark values.
- **Tests cover both palettes**: the playing row is distinct from both stripes and from hover in each, foregrounds and backgrounds are on opposite sides of a luminance split, and the accent is checked against the mode it has to be legible in. **Mode-mutating tests hold `TEST_MODE_LOCK`**, and so do the colour assertions elsewhere — the mode is global and tests run in threads, so without it a flip lasting microseconds can make an unrelated assertion read the wrong palette.
- **Not done: "follow the system".** Needs OS detection (the `dark-light` crate); Light/Dark first.
...
**Transport button geometry is load-bearing, not cosmetic.** The glyphs first shipped rendering as **nothing** — blank buttons, not tofu — because `button`'s default padding of 5 left an 18px content box for a 17px glyph whose line box needs ~22px, and iced draws no line at all rather than a clipped one. `TRANSPORT_H - 2*TRANSPORT_PAD_Y` must exceed `TRANSPORT_ICON * 1.3` by `TRANSPORT_MIN_SLACK`; `transport_glyph_fits_its_button` pins it, with slack because iced rounds line boxes and a design that only *just* fits is one rounding from the same bug. A second test keeps the glyph/button ratio in a band, since the first version fitted fine and still read as a speck in a wide box. Blank-not-tofu is the tell that a glyph *didn't fit*, as opposed to a missing font.
...
- **`Message::AlbumArtFetched(key, ArtOutcome)`** is the queue's own message; `ArtLoaded` is now only for one-off fetches (playing track, artist images, recents).
- **The one-off path has its own in-flight set, `oneoff_art_pending`, and `fetch_art`/`fetch_artist_art` own *both* guards** (already-loaded and already-running) rather than leaving the first to each call site and the second to nobody. `CurrentSongUpdated` fires on every 500ms status poll and its only guard was `art_handles`, which is populated when a fetch *finishes* — so a cover that takes seconds to arrive, i.e. anything reaching MusicBrainz at ~1.1s of globally serialized throttle, was re-requested on every poll until the first landed. Measured against the real server on a cold cache: **the same album fetched four times, 1.2s apart; one after the fix.** Each duplicate also held a throttle slot the background sweep needed. `Message::ArtLoaded` releases the key **on the miss path too** — a fetch that found nothing has still finished. `App::claim_oneoff_art` is the pure guard (an `App` can't be built in a test: `AppConfig::load()` resolves the real user config path); an already-loaded key is refused *without* being inserted, since nothing would ever remove it. Deliberately **not** cleared by `SwitchServer` or `ClearCaches`: it is duplicate suppression, not a work queue, so clearing it would permit exactly the second fetch it exists to prevent, and it self-cleans because every one-off fetch ends in an `ArtLoaded`. This replaced an earlier trick of inferring the source from `art_pending` membership — the stage split needs a real tri-state (`MpdMiss` is "not found yet", `Missing` is "final"), which a bare `Option<Vec<u8>>` can't carry.
...
## Search sections + batch select (`src/mpd/types.rs`, `src/ui/views/search.rs`)
Search renders **three sections** — Artists, Albums, Songs — instead of one flat song list under per-album headers (`library-album-identity-and-multidisc.md` Part D, deferred since 0.4.1, shipped in 0.5.0).
- **The sections are derived from the one result set, not fetched by three queries** — the plan's one deviation. `search any` has already matched every tag, so the artists and albums are in the reply; three queries would cost three round trips on the one shared connection to rediscover them and could disagree with the songs on screen if the database changed between them. `search_sections(results, query)` is a pure function in `types.rs`, so all of it is unit-testable.
- **A section lists an entity only when its *own identity* matches** — for an artist the name, for an album the title **or its artist**. The album half is the 0.5.1 fix: matching on the title alone meant searching an artist listed almost none of their records, and `Metallica` returned 195 songs beside **two** album rows — the self-titled one and someone else's *Plays Metallica By Four Cellos* — hiding the dozen albums those songs came from (14 after the fix). What neither half does is match on a *song*: a search for `love` must not promote all 300 artists who recorded a song with "love" in the title, nor their whole discographies, into the sections. The Songs section is the one that keeps every match. Matching is `folded_contains`, which folds by the same rules as `album_grouping_key`, so a typed hyphen finds an en dash.
- **Albums go through `group_albums_by_artist`**, so the section is disc-collapsed and artist-scoped like every other album list, carries the same "N discs" caption, and draws the same covers (`View::Search` is a `queue_album_art` target; results arrive *after* view entry, so `SearchResults` kicks the queue rather than `on_view_enter`).
- **Artists are de-duplicated on the exact spelling, deliberately, even though matching is folded.** Each distinct spelling is a distinct tag holding distinct songs — this library really has both `"Blue Oyster Cult"` and `"Blue  Oyster Cult"` — and an artist row navigates by name, so folding the two into one row would leave the other's albums unreachable. The Artists list has the same duplicates for the same reason.
- **Batch select**: a checkbox per song row plus a select-all in the Songs header, driving `search_selected: HashSet<String>` on `App`. The action bar (`N selected` · Add to queue · Add to playlist… · Clear) appears **only when something is ticked** — an always-present row of disabled buttons is noise on every search that doesn't use them. Queueing goes through `add_all` (one `command_list`, per the bulk-enqueue rule) and reports partial success as a toast, since `command_list` stops at the first failure. **The selection is cleared on every new result set**: carrying it across a search would leave the count reading N while the ticked rows are off screen, and "Add to queue" would enqueue the *previous* query's tracks.
...
### Missing files in playlists (ported from mikMPD v1.7)
Stored playlists outlive their files. `listplaylistinfo` returns an entry whose file is gone as a **bare `file:` line** — no tags, no duration, no `Last-Modified` — and `load` **silently skips it**, moving every later row up one place.
- **`PlaylistPlayAt(name, pos)` carries the row's *playlist* index, and the handler maps it to a *queue* position** via `playlist_queue_index(index, songs)` (`types.rs`) before sending `play`. It used to send `play pos`, which lands on the wrong song for every row below the first dead entry — on mikMPD's real 417-entry playlist with five, **410 of 412** playable rows. The mapping lives in the handler, not the view, so the message keeps meaning "this row". `PlaylistPlay` needs no mapping: queue position 0 is the first entry `load` kept.
- **`Song::is_missing_from_library()` is the detection**: a library URI *and* no `last_modified` *and* no (or zero) duration. Requiring all three keeps a real song missing just one signal from being flagged. **Any `scheme://` URI is excluded, not only mikMPD's `http`/`https`/`icy`/`cdda`** — `load` *keeps* streams, so an `rtsp://` entry counted as skipped would shift the mapping for every row after it.
- **`PlaylistDetail` shows the entry rather than hiding it** — warning glyph, filename, "Missing file · {folder}" (the folder is what someone restoring the file needs) — so the list still matches what was saved. Play, Add to queue, Play next and Add to playlist are **disabled, not omitted** (column alignment, as everywhere); Move and Remove stay live. mikMPD's tap-to-explain dialog is a **tooltip** here, and its Remove option is the row's own Remove button. The header counts what Play will play separately (`412 tracks  |  5 missing  |  … min`) and adds a one-line note; a playlist with nothing missing renders exactly as before.
- **Header art comes from `playlist_cover_song`, the first non-missing entry** — used by both the fetch and the view's lookup. A missing entry's art key is the `Unknown Artist`/`Unknown Album` placeholder, so a playlist starting with one showed no cover.
- `live_playlist_rows_play_the_song_load_puts_there` is the acceptance test: it `load`s every playlist holding a flagged entry into a scratch partition and asserts the queue is exactly the unflagged entries and every row maps to its own file. It passes vacuously on a server with no such playlist, since `playlistadd` refuses a file the database lacks, so one can't be constructed. **Verified against the 10.0.1.3 server (2026-09-11)**: `Bra grejs` (417 entries, 6 missing) and `Spellista 1` (5 entries, 3 missing) both load exactly as detected, and all 411 + 2 playable rows map to their own file — where play-by-row-index got **409 of 411** wrong. That server now denies `partition` without a password, so the scratch-partition tests need `WINRMPC_TEST_MPD_PASSWORD`.
...
Wayland resolves an icon by matching the surface's `app_id` to a `.desktop` basename, so `main.rs` sets `platform_specific.application_id` from `icon_design::APP_ID` (`io.github.mickegris.winrmpc`). `PlatformSpecific` is a **different type per OS**, hence the cfg-split `platform_specific()` helper next to `window_settings()`. That id must stay byte-identical in three places — the `app_id`, the `.desktop` filename, and the icon filenames — or the icon silently doesn't resolve. **`packaging/linux/install.sh` has to find the binary in two different layouts.** In a git checkout it is `target/release/winrmpc`; in the **released tarball** it is the script's *parent* directory and there is no `target/` at all. It knew only the first, so anyone installing from a prebuilt tarball was told to "run `cargo build --release` first" and ended up with icons, a `.desktop` entry, and **no executable on PATH** — a launcher icon that does nothing when clicked. It now probes tarball → checkout → beside-the-script.

**macOS is packaged as of 0.4.3** — `packaging/macos/bundle.sh` builds a universal `winrmpc.app` with an `.icns` generated from the same `icon_design`, and CI attaches it to releases. The `.app` is the *only* way macOS gets a Dock icon, since `set_window_icon` is a no-op there. Three things about it: `CFBundleShortVersionString` is substituted from `Cargo.toml` by the script rather than committed (a literal would go stale like the MusicBrainz UA did); `CFBundleIdentifier` is `icon_design::APP_ID` and is deliberately **not** the `ProjectDirs` id that names the config directory — aligning them would orphan every existing install's settings; and the bundle is **unsigned**, so macOS reports it as *damaged* until `xattr -dr com.apple.quarantine` is run — Gatekeeper's wording for an unsigned download, not a corrupt file. See `docs/plans/macos-app-bundle.md`. **Signing is still planned** now that a paid Apple account exists (it slipped 0.4.5 and 0.5.0 — the repo-side work is ready to write, but the account-side step A has not been done: `gh secret list` is empty) (`docs/plans/macos-signing-and-notarization.md`); note that notarisation additionally requires the hardened runtime and a secure timestamp, that the ticket has to be **stapled** into the bundle (so the zip is built twice — once to submit, once after stapling), and that four places currently document the `xattr` workaround and must all change together.
...
`macos-signing-and-notarization.md` is the still-open follow-up to `macos-app-bundle.md` — Developer ID signing, notarisation and stapling, plus the account-side prerequisites that can't live in the repo.

`ui-0.4.3.md` is the newest umbrella (targeting 0.4.3, **plans only so far**), linking eight plans: `icon-buttons-and-transport-controls.md`, `clickable-artist-album-links.md`, `theme-light-and-dark.md`, `macos-app-bundle.md`, `keyboard-shortcuts.md`, `album-highlighting-and-scroll-to-playing.md`, `error-and-status-feedback.md`, `window-and-session-state.md`. Four were requested and four proposed; two of them finish work deferred out of 0.4.2 (`current-song-highlighting.md` steps D/E, `app-icon-cross-platform.md` steps D/E). Its "Considered and not planned" section records the ideas that were weighed and left out, so they don't get rediscovered from scratch.
...
## Current Version
`0.5.2` — see `Cargo.toml`. There are `release` and `ship` skills that automate the release/merge flow — prefer them over doing the steps by hand.
