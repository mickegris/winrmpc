...
- **Tests**: inline `#[cfg(test)] mod tests` blocks (this is a *binary* crate — a top-level `tests/` dir can't reach internal/private items like `escape` and `parse_ack`). 268 offline tests; none touch the network or need an MPD server (the `store` ones do open a real redb, over `InMemoryBackend`):
...
`macos-signing-and-notarization.md` is the 0.4.5 follow-up to `macos-app-bundle.md` — Developer ID signing, notarisation and stapling, plus the account-side prerequisites that can't live in the repo.
