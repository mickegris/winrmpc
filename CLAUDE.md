...
- ⚠️ **Known bug**: `album_songs`/`album_bios` are still keyed by base name alone (not artist-scoped) — two different artists' same-named album collide in those caches. The key shape pre-dates this work, but making same-named albums *separately reachable* is exactly what this change did, so it's now hit in normal use: `fetch_album_bio`'s `contains_key(&album)` guard means the second artist's page shows the **first artist's bio for the whole session** (the redb key *is* artist-scoped; only the in-memory map isn't). `album_songs` self-corrects because its load always fires. Fix planned in [`docs/plans/review-fixes-correctness.md`](docs/plans/review-fixes-correctness.md) §2.
- ⚠️ **Known bug**: the art-cache key sites for *recently-played* albums were not updated to strip disc markers alongside `art_key()`, so they miss the cache and burn throttled MusicBrainz lookups. See [`review-fixes-correctness.md`](docs/plans/review-fixes-correctness.md) §1.
- ⚠️ **Known bug**: `album_base_and_disc` can panic on album titles whose lowercase changes byte length (e.g. Turkish `İ`), and it's reachable from `art_key()` inside `view()`. See [`review-fixes-correctness.md`](docs/plans/review-fixes-correctness.md) §4.
- **Not yet done** (scoped out of the initial pass, see `docs/plans/library-album-identity-and-multidisc.md`): grid view (Part C) and Search sections/batch-select (Part D).
...
- **Connection lifecycle**: `self.snapcast_client: Option<SnapcastClient>` is created lazily on the **first** `View::Snapcast` visit (`on_view_enter`, now `&mut self`) and the *client value* is kept across later visits rather than torn down on navigation-away. ⚠️ **Two known bugs here, both written up in [`docs/plans/review-fixes-correctness.md`](docs/plans/review-fixes-correctness.md) §3 and [`review-fixes-performance.md`](docs/plans/review-fixes-performance.md) §3 — do not treat the current behavior as intended**: (1) `SnapcastClient::connect()` unconditionally opens a *new* `TcpStream` and drops the old one, so every view visit reconnects (an in-code comment claiming it's "no-op-ish cheap" is wrong); (2) the `is_none()` guard means the client keeps the address of whichever server was active on the **first** visit — `SwitchServer` does not reset it, so after switching servers the Snapcast view still controls the old machine.
