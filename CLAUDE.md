...
- **Tests**: inline `#[cfg(test)] mod tests` blocks (this is a *binary* crate — a top-level `tests/` dir can't reach internal/private items like `escape` and `parse_ack`). 262 offline tests; none touch the network or need an MPD server (the `store` ones do open a real redb, over `InMemoryBackend`):
...
- `sort_desc: bool` — Z–A instead of A–Z for every name-sorted list, `#[serde(default)]`; see "List sorting"
...
| `sort_desc` | the A–Z / Z–A toggle in each list header |
...
Six lists carry an **A–Z / Z–A** toggle: Artists, Albums, Genres, Playlists, and the album lists on Artist and Genre detail. One persisted flag (`AppConfig::sort_desc`) drives all of them — the same call `album_grid_view` made, since independent per-list sort memories would feel arbitrary.
- **`name_cmp` is not `str::cmp`, and that was a real bug.** A plain `Vec<String>::sort()` is byte order, so every lowercase initial filed after every uppercase one: `ZZ Top` (`Z` = 0x5A) sorted before `a-ha` (`a` = 0x61). `a-ha`, `dEUS`, `k.d. lang` and `will.i.am` were all exiled past Z in the Artists list. `name_cmp` case-folds, **then breaks ties on bytes** — the tiebreak keeps the order *total*, without which names differing only in case have no defined order and reversing twice shuffles the list instead of restoring it. Two tests pin exactly that involution.
- **Sorting happens in `update`, never in `view()`.** Views take `&'a [T]` and would have to allocate a reordered copy every frame. `App::sort_library_lists()` is called from both entry points — every `*Loaded` handler and `ToggleSortDirection` — so the two can't drift onto different comparators. `ToggleSortDirection` **re-sorts in memory and does not refetch**: the data is in hand, and a round trip to reverse a list the user is looking at would show as a flicker.
- **The recency lists are deliberately absent from it.** `recently_added_albums` and `recently_played` are ordered by time, which is the only thing they are for. `albums_list::view` is shared with Recently Added, so its `sort_desc` param is `Option<bool>` rather than `bool` — `None` means the control is **absent**, not present and doing nothing.
...
- **`link::sort_toggle` is text, not a glyph.** The obvious reuse is barred anyway — `icon::MOVE_UP`/`MOVE_DOWN` already *are* `arrow_upward`/`arrow_downward`, and `icon::tests` asserts no two constants share a codepoint — but the real reason is that an arrow doesn't say *what* is being sorted while "A–Z" does. Same argument as Single/Consume keeping their words.
